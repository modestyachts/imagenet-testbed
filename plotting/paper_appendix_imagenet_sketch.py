...
@click.option('--y_axis', type=str, default='imagenet-sketch')
...
def generate_xy_plot(x_axis, y_axis, transform, num_bootstrap_samples, output_dir, output_file_dir, skip_download):
    ...
    ylim = [df_visible[y_axis].min() - 0.5, df_visible[y_axis].values.max() + 2]
    ...
    fig, _ = plotter.model_scatter_plot(df, x_axis, y_axis, xlim, ylim, ModelTypes, 
                                         ...
                                         title='Distribution Shift to ImageNet-Sketch', x_label='ImageNet', y_label='ImageNet-Sketch', 
                                         figsize=(12, 8), include_legend=False, return_separate_legend=False)
    
    os.makedirs(output_file_dir, exist_ok=True)

    fig.savefig(join(output_file_dir, f'imagenet_sketch.pdf'), dpi='figure', bbox_inches='tight', pad_inches=0.1)
    print(f"Plot saved to {join(output_file_dir, f'imagenet_sketch.pdf')}")
