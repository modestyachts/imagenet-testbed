...
def show_in_plot(df_row, option):
    ...
    if option == 'only-standard':
        return 'subsample' not in model_name and df_row['val-on-ytbb-robust-classes'] >= 95 and model_type == NatModelTypes.STANDARD
    if option == 'no-subsample':
        return 'subsample' not in model_name and df_row.val >= 55        
    return 'subsample' not in model_name and df_row['val-on-ytbb-robust-classes'] >= 95
