...
@click.option('--y_axis', type=str, default='avg_pgd')
...
def generate_xy_plot(x_axis, y_axis, y_axis_fit, transform, num_bootstrap_samples, output_dir, output_file_dir, skip_download):
    ...
    fig.savefig(join(output_file_dir, f'syn_shift_pgd.pdf'), dpi='figure', bbox_inches='tight', pad_inches=0.1)
    print(f"Plot saved to {join(output_file_dir, f'syn_shift_pgd.pdf')}")
    ...
    fig.savefig(join(output_file_dir, f'eff_robust_pgd.pdf'), dpi='figure', bbox_inches='tight', pad_inches=0.1)
    print(f"Plot saved to {join(output_file_dir, f'eff_robust_pgd.pdf')}")
