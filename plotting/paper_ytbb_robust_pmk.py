...
def show_in_plot(df_row, option):
    ...
    if option == 'only-standard':
        return 'subsample' not in model_name and df_row['ytbb-robust_pm0'] >= 45 and model_type == ModelTypes.STANDARD
    return 'subsample' not in model_name and df_row['ytbb-robust_pm0'] >= 45
...
@click.command()
@click.option('--x_axis', type=str, default='ytbb-robust_pm0')
@click.option('--y_axis', type=str, default='ytbb-robust_pm10')
...
def generate_xy_plot(x_axis, y_axis, transform, num_bootstrap_samples, output_dir, output_file_dir, skip_download, option):
    ...
    if option == 'show-yx':
        ...
        ylim = [df_visible[y_axis].min() - 2, df_visible[y_axis].values.max() + 3]
    ...
    fig, _ = plotter.model_scatter_plot(df, x_axis, y_axis, xlim, ylim, ModelTypes, 
                                         ...
                                         title='Distribution Shift to YTBB-Robust', x_label='YTBB-Robust', y_label='YTBB-Robust', 
                                         figsize=(12, 8), include_legend=False, return_separate_legend=False)
  
    os.makedirs(output_file_dir, exist_ok=True)

    filename = 'ytbb_robust_pmk'
    if option == 'only-standard':
        filename += '_standard'
