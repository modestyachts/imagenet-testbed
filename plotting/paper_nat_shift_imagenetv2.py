...
def show_in_plot(df_row, option):
    ...
    if option == 'only-standard':
        return 'subsample' not in model_name and df_row.val >= 55 and model_type == NatModelTypes.STANDARD
...
def generate_xy_plot(x_axis, y_axis, transform, num_bootstrap_samples, output_dir, output_file_dir, skip_download, option):
    ...
    if option == 'show-yx':
        ylim = [df_visible[y_axis].min() - 1, df_visible[[x_axis, y_axis]].values.max() + 1]
    else:
    ...
    fig, _, legend = plotter.model_scatter_plot(df, x_axis, y_axis, xlim, ylim, NatModelTypes, 
                                         ...
                                         title='Distribution Shift to ImageNetV2', x_label='ImageNet', y_label='ImageNetV2', 
                                         figsize=(12, 8), include_legend=False, return_separate_legend=True)

    os.makedirs(output_file_dir, exist_ok=True)

    if not option:
        legend.savefig(join(output_file_dir, f'nat_shift_legend.pdf'), dpi='figure', bbox_inches='tight', pad_inches=0.1)
        print(f"Legend saved to {join(output_file_dir, f'nat_shift_legend.pdf')}")
    if option=='only-standard':
        legend.savefig(join(output_file_dir, f'standard_legend.pdf'), dpi='figure', bbox_inches='tight', pad_inches=0.1)
        print(f"Legend saved to {join(output_file_dir, f'standard_legend.pdf')}")        

    filename = 'nat_shift_imagenetv2'
    if option == 'only-standard':
        filename += '_standard'
