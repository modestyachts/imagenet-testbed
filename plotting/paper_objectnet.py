...
def show_in_plot(df_row, option):
    ...
    if option == 'only-standard':
        return 'subsample' not in model_name and df_row['val-on-objectnet-classes'] >= 55 and model_type == NatModelTypes.STANDARD
    if option == 'no-subsample':
        return 'subsample' not in model_name and df_row.val >= 55        
    return 'subsample' not in model_name and df_row['val-on-objectnet-classes'] >= 55
...
@click.command()
@click.option('--x_axis', type=str, default='val-on-objectnet-classes')
@click.option('--y_axis', type=str, default='objectnet-1.0-beta')
...
def generate_xy_plot(x_axis, y_axis, transform, num_bootstrap_samples, output_dir, output_file_dir, skip_download, option):
    ...
    if option == 'show-yx':
        ...
        ylim = [df_visible[y_axis].min() - 1, df_visible[y_axis].values.max() + 2]
    ...
    fig, _ = plotter.model_scatter_plot(df, x_axis, y_axis, xlim, ylim, NatModelTypes, 
                                         ...
                                         title='Distribution Shift to ObjectNet', x_label=x_label, y_label='ObjectNet', 
