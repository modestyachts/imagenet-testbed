...
# THIS SECTION OF THE CODE IS TAKEN FROM https://github.com/facebookresearch/swav
...
def classifier_loader():
    backbone = resnet50(output_dim=0, eval_mode=True)
    linear_classifier = RegLog(1000, arch='resnet50', global_avg=True, use_bn=False)
    model = nn.Sequential(backbone, linear_classifier)
    load_model_state_dict(model, 'resnet50_swav')
    return model
