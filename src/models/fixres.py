...
'FixPNASNet': {   'arch': 'pnasnet5large',
                  ...
                  'mean': [0.5, 0.5, 0.5],
                  'std': [0.5, 0.5, 0.5]},
'FixResNeXt101_32x48d': {   'arch': 'resnext101_32x48d',
                            'eval_batch_size': 2,
                            'img_crop_size': 320,
                            'img_resize_size': 365,
                            'input_space': 'RGB',
...
'FixResNeXt101_32x48d_v2': {   'arch': 'resnext101_32x48d',
                               'eval_batch_size': 2,
                               'img_crop_size': 320,
                               'img_resize_size': 365,
                               'input_space': 'RGB',
...
'FixResNet50': {   'arch': 'resnet50',
                   ...
                   'mean': [0.485, 0.456, 0.406],
                   'std': [0.229, 0.224, 0.225]},
'FixResNet50CutMix': {   'arch': 'resnet50',
                         'eval_batch_size': 64,
                         'img_crop_size': 320,
                         'img_resize_size': 365,
                         'input_space': 'RGB',
                         'mean': [0.485, 0.456, 0.406],
                         'std': [0.229, 0.224, 0.225]},
'FixResNet50CutMix_v2': {   'arch': 'resnet50',
                            'eval_batch_size': 64,
                            'img_crop_size': 320,
                            'img_resize_size': 365,
                            'input_space': 'RGB',
                            'mean': [0.485, 0.456, 0.406],
                            'std': [0.229, 0.224, 0.225]},
'FixResNet50_no_adaptation': {   'arch': 'resnet50',
                                 'eval_batch_size': 256,
                                 'img_crop_size': 224,
                                 'img_resize_size': 256,
                                 'input_space': 'RGB',
...
# Code is adapted from https://github.com/facebookresearch/FixRes
...
def gen_classifier_loader(name, d):
    def classifier_loader():
        ...
        elif d['arch'] == 'resnext101_32x48d':
            model = torch.hub.load('facebookresearch/WSL-Images', d['arch']+'_wsl')
        elif d['arch'] == 'pnasnet5large':
            model = pnasnet5large(num_classes=1000, pretrained=False)
...
for name, d in model_params.items():
    registry.add_model(
        Model(
            name = name,
            arch = d['arch'],
            transform = transforms.Compose([Resize(d['img_resize_size']), 
                                            transforms.CenterCrop(d['img_crop_size']), 
                                            transforms.ToTensor()]),
