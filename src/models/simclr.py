...
from models.model_base import Model, StandardTransform
from mldb.utils import load_model_state_dict


model_params = {
'resnet50_simclrv2_linear_probe_tf_port': {     'arch': 'resnet50',
                                                'eval_batch_size': 256,
                                                'depth': 50,
                                                'width': 1,
                                                'sk_ratio': 0},
'resnet50_simclrv2_finetuned_100pct_tf_port': { 'arch': 'resnet50',
                                                'eval_batch_size': 256,
                                                'depth': 50,
                                                'width': 1,
                                                'sk_ratio': 0},
'resnet152_3x_simclrv2_linear_probe_tf_port': {     'arch': 'resnet152_3x',
                                                    'eval_batch_size': 32,
                                                    'depth': 152,
                                                    'width': 3,
                                                    'sk_ratio': 0.0625},
'resnet152_3x_simclrv2_finetuned_100pct_tf_port': { 'arch': 'resnet152_3x',
                                                    'eval_batch_size': 32,
                                                    'depth': 152,
                                                    'width': 3,
                                                    'sk_ratio': 0.0625}}
...
# THIS SECTION OF THE CODE IS TAKEN FROM https://github.com/Separius/SimCLRv2-Pytorch/
...
def gen_classifier_loader(name, d):
    def classifier_loader():
        model, _ = get_resnet(d['depth'], d['width'], d['sk_ratio'])
...
for name, d in model_params.items():
    registry.add_model(
        Model(
            ...
            classify = lambda model, images: model(images, apply_fc=True)
        )
    )
